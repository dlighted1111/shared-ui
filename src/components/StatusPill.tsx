...
export function StatusPill({ label, color = "gray", className }: StatusPillProps) {
  const colors = STATUS_COLORS[color] ?? STATUS_COLORS.gray;

  return (
    <span
      className={cx("inline-flex items-center rounded-[4px] px-2 py-[2px] text-xs font-medium whitespace-nowrap", className)}
